...
#pragma warning restore CS0414
    ...
    // Class synergies used to be granted here, in parallel with a *different* rule that the
    // assembler screen displayed (same species, via AnimalData.canStack). The two never
    // agreed, so the panel could read "no synergies" on a buffed team. Synergies are now
    // evaluated once by TeamSynergy and applied at spawn by CombatTeamSpawner, so the
    // screen and the battle cannot drift apart.
