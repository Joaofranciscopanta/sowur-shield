...
- [x] ~~**`ShopUI` is blocked on content**~~ — **done Aug/15, and the Aug/1 diagnosis above was
  wrong on both counts.** There were never "zero `ShopData` assets": `PlaceholderShopTool` had
  already authored four (`clara`, `isabela`, `rui`, `tomas`) into `Resources/Shops/` **and
  assigned them to the matching NPCs in SampleScene**. The scene search that reported "no
  `ShopNPC` placed" had matched `BuildingShopNPC` on the suffix — a different component for a
  different shop. So the content and the NPC wiring were both already done.
  **Three separate defects kept it unreachable, each silent:**
  1. **No `ShopUI` object in the scene, and no builder to make one.** `NPCDialogueInteractable`
     resolves it with `FindFirstObjectByType` behind an `if (shopUI != null)`, so picking
     "browse shop" closed the dialogue and did nothing at all. Fixed with `ShopUIBuilder.cs`
     (`Tools > Sowur Shield > Rebuild Shop UI`), modelled on `BuildingShopUIBuilder`.
  2. **`ShopItemRow` was declared as a second class inside `ShopUI.cs`.** Unity creates one
     MonoScript per *file*, named after the file, so the type had no script asset to reference:
     adding it to a prefab saved a **missing-script** component with no error. Any shop-row
     prefab would always have been born broken. Moved to `ShopItemRow.cs`, mirroring
     `BuildingRow`, which had its own file all along.
  3. **`BuildItemRows` used `Destroy`, so rows accumulated across shops.** Deferred to end of
     frame, the old rows were still children while the new ones spawned — measured 4 → 8 → 12 →
     15 items when switching between the four shops in one session. Now `DestroyImmediate`,
     verified 4/4/4/3 over two full rotations.
  **Verified in Play Mode**, not by reading: all four shops populate with localized names, the
  relationship discount applies (tomás at 25 affinity → 120g listed as 114g), and a real click
  on the row's Buy button moved −114 gold / +1 Hoe. Console clean; 815 EditMode + 63 PlayMode
  green. **Still cosmetic-only:** the panel uses the theme's runtime restyle but its layout has
  not had a screenshot pass, and item icons are the existing placeholders
...
~~row prefabs for `ShopUI`~~ **done Aug/15** — `Resources/Prefabs/UI/ShopItemRow.prefab` is
generated by `ShopUIBuilder`, which also fixed the reason one could never be authored by hand
(see the `ShopUI` entry above). `Resources/Quests/` has 4 quests
